---
title: "Building Modern AI Interfaces"
date: "2026-07-30"
updatedAt: "2026-08-28"
description: "Exploring patterns for integrating LLMs into production web applications with streaming, error handling, and user feedback loops."
tags: ["AI", "React", "System Design"]
series: "AI Interface Engineering"
topic: "ai-interface"
author: "Ethan Pier"
environment: "Next.js 16.3, React 19.2, Node.js 22"
---

> **Example scope**: The following snippets are cumulative and target a browser client component in Next.js 16.3, React 19.2, and Node.js 22. `/api/chat` must be provided by your own same-origin Route Handler with a streaming `Response`. The example covers cancellation, HTTP errors, and empty responses, but not authentication, retries, schema validation, protocol parsing, or production error reporting.

## The Challenge

Integrating Large Language Models into production web applications isn't just about calling an API. It requires rethinking how we handle:

- **Streaming responses** — Users expect real-time feedback, not a loading spinner for 10 seconds.
- **Error boundaries** — LLMs are non-deterministic. Your UI needs graceful degradation.
- **Token management** — Context windows have hard limits. Smart truncation preserves conversation quality.

## Streaming Architecture

The foundation of any AI interface is a robust streaming pipeline:

```typescript {5,8,12,28}
async function* streamResponse(prompt: string, signal: AbortSignal) {
  const response = await fetch('/api/chat', {
    method: 'POST',
    body: JSON.stringify({ prompt }),
    signal,
  });

  if (!response.ok) {
    throw new Error(`Chat request failed: ${response.status}`);
  }

  if (!response.body) {
    throw new Error('Chat response has no readable body');
  }

  const reader = response.body.getReader();
  const decoder = new TextDecoder();

  try {
    while (true) {
      const { done, value } = await reader.read();
      if (done) break;
      yield decoder.decode(value, { stream: true });
    }
    const trailing = decoder.decode();
    if (trailing) yield trailing;
  } finally {
    await reader.cancel().catch(() => undefined);
    reader.releaseLock();
  }
}
```

This generator pattern composes cleanly with React's state model and allows for cancellation, retry, and backpressure handling.

## State Machine for Chat

Rather than ad-hoc boolean flags, model conversation state as a finite state machine:

```typescript
type ChatState =
  | { status: 'idle' }
  | { status: 'streaming'; buffer: string }
  | { status: 'error'; error: Error; lastMessage: string }
  | { status: 'complete'; response: string };
```

Each state transition is explicit, testable, and maps directly to UI states. No more `isLoading && !isError && hasResponse` chains.

## React Integration Pattern

Place this hook in the same `.tsx` file as the preceding `streamResponse` and `ChatState` examples. The imports and reducer are included here so the hook does not hide undefined dependencies behind a seemingly complete example. Call the hook from a client component and render send, cancel, and error states from `state.status`.

```tsx
import { useCallback, useReducer, useRef } from 'react';

type ChatAction =
  | { type: 'START' }
  | { type: 'CHUNK'; payload: string }
  | { type: 'COMPLETE' }
  | { type: 'ERROR'; payload: Error }
  | { type: 'CANCEL' };

function chatReducer(state: ChatState, action: ChatAction): ChatState {
  switch (action.type) {
    case 'START':
      return { status: 'streaming', buffer: '' };
    case 'CHUNK':
      return state.status === 'streaming'
        ? { status: 'streaming', buffer: state.buffer + action.payload }
        : state;
    case 'COMPLETE':
      return state.status === 'streaming'
        ? { status: 'complete', response: state.buffer }
        : state;
    case 'ERROR':
      return { status: 'error', error: action.payload, lastMessage: state.status === 'streaming' ? state.buffer : '' };
    case 'CANCEL':
      return { status: 'idle' };
  }
}

function useAIStream() {
  const [state, dispatch] = useReducer(chatReducer, { status: 'idle' });
  const abortRef = useRef<AbortController | null>(null);

  const send = useCallback(async (prompt: string) => {
    abortRef.current?.abort();
    abortRef.current = new AbortController();

    dispatch({ type: 'START' });

    const signal = abortRef.current.signal;

    try {
      for await (const chunk of streamResponse(prompt, signal)) {
        dispatch({ type: 'CHUNK', payload: chunk });
      }
      dispatch({ type: 'COMPLETE' });
    } catch (error) {
      if (error instanceof Error && error.name !== 'AbortError') {
        dispatch({ type: 'ERROR', payload: error });
      }
    } finally {
      if (abortRef.current?.signal === signal) {
        abortRef.current = null;
      }
    }
  }, []);

  const cancel = useCallback(() => {
    abortRef.current?.abort();
    dispatch({ type: 'CANCEL' });
  }, []);

  return { state, send, cancel };
}
```

## Key Takeaways

1. **Stream from day one** — retrofitting streaming is painful.
2. **Design for failure** — every LLM call can timeout, rate-limit, or hallucinate.
3. **Measure latency per-token**, not just time-to-first-byte.
4. **Build cancellation into the contract** — users will navigate away mid-stream.
5. **Type your states** — state machines prevent impossible UI combinations.
