...
export function CurrentsDailyBody({ report, locale, labels }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl py-14">
      ...
      {/* 头条 */}
      {report.lead && (
        <TransitionLink href={`/currents/${report.lead.id}`} className="currents-surface-highlight mb-10 block rounded-xl p-6">
          ...
          <section key={section.key} className="mb-10">
            ...
            <ul className="space-y-4">
              {section.items.map((item) => (
                <li key={item.id}>
                  <TransitionLink href={`/currents/${item.id}`} className="group flex items-start justify-between gap-3">
